...
public class Controller implements ActionListener, GUIInformation {
	...
	/**
	 * Create a controller with the argument
	 * If no argument is provided, the controller will initialize an empty editor
	 */
	public Controller() {
	...
	public Controller(int level, String name){
		// If the argument is a .xml file, initialize an empty editor and load the map from the file
		if(level == 0){
			init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
			loadFile(name);

			// If a folder name is passed as the argument, validate the folder and its files,
			// then store all maps and run them sequentially
		}else if(level == 1){
			ArrayList<PacmanMap> maps = runMap.loadAndCheckMaps(name);
